...
function formatEmailBodyWithLinks(text) {
    ...
    return escapedText.replace(urlRegex, '<a href="$1" target="_blank" rel="noopener noreferrer" style="color: #0066cc; text-decoration: underline;">$1</a>');
}
...
// 话多型KYC对话
function generateVerboseKYCConversation(customerName, customerAge, platform, additionalInfo, willProvide) {
    const messages = [];
    const now = new Date();
    const day1 = new Date(now);
    day1.setDate(day1.getDate() - 2);
    ...
    if (willProvide) {
        ...
        messages.push({
            sender: 'customer',
            text: `Will do. I'll send them over today.`,
            time: formatTime(day1, 10, 46)
        });
    } else {
        const refusalReason = generateRefusalReason(customerName);
        messages.push({
...
// 话少型KYC对话
function generateConciseKYCConversation(customerName, customerAge, platform, additionalInfo, willProvide) {
    const messages = [];
    const now = new Date();
    const day1 = new Date(now);
    day1.setDate(day1.getDate() - 2);
    ...
    if (willProvide) {
        messages.push({
            sender: 'customer',
            text: `Okay, where should I send them?`,
...
// 谨慎型KYC对话
function generateCautiousKYCConversation(customerName, customerAge, platform, additionalInfo, willProvide) {
    const messages = [];
    const now = new Date();
    const day1 = new Date(now);
    day1.setDate(day1.getDate() - 2);
    ...
    messages.push({
        ...
        time: formatTime(day1, 9, 30)
    });

    if (willProvide) {
        messages.push({
            sender: 'customer',
            text: `I understand. What security measures do you have in place for storing these documents?`,
            time: formatTime(day1, 9, 33)
        });

        messages.push({
            sender: 'company',
            text: `All documents are encrypted and stored in secure, compliant systems. We follow strict data protection protocols.`,
            time: formatTime(day1, 9, 36)
        });

        messages.push({
            sender: 'customer',
            text: `Okay, I'll provide them. How should I send them?`,
            time: formatTime(day1, 9, 39)
        });
...
// 急切型KYC对话
function generateUrgentKYCConversation(customerName, customerAge, platform, additionalInfo, willProvide) {
    const messages = [];
    const now = new Date();
    const day1 = new Date(now);
    day1.setDate(day1.getDate() - 2);
...
// 友好型KYC对话
function generateFriendlyKYCConversation(customerName, customerAge, platform, additionalInfo, willProvide) {
    const messages = [];
    const now = new Date();
    const day1 = new Date(now);
    day1.setDate(day1.getDate() - 2);
    ...
    if (willProvide) {
        messages.push({
            sender: 'customer',
            text: `Sure, no problem! How should I send them?`,
            time: formatTime(day1, 10, 18)
        });
        ...
        messages.push({
            sender: 'customer',
            text: `Will do! Sending them over now.`,
            time: formatTime(day1, 10, 24)
        });
    } else {
        const refusalReason = generateRefusalReason(customerName);
        messages.push({
...
// 专业型KYC对话
function generateProfessionalKYCConversation(customerName, customerAge, platform, additionalInfo, willProvide) {
    const messages = [];
    const now = new Date();
    const day1 = new Date(now);
    day1.setDate(day1.getDate() - 2);
    ...
    if (willProvide) {
        messages.push({
            sender: 'customer',
            text: `Understood. What's the preferred method of submission?`,
            time: formatTime(day1, 9, 3)
        });
        ...
        messages.push({
            sender: 'customer',
            text: `Will do. I'll submit them today.`,
            time: formatTime(day1, 9, 9)
        });
    } else {
        const refusalReason = generateRefusalReason(customerName);
